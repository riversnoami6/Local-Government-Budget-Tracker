...
(define-public (create-budget
  ...
  (let ((budget-id (var-get next-budget-id)))
    (begin
      ...
      (asserts! (default-to false (map-get? department-admins tx-sender))
        ERR_UNAUTHORIZED
      )
...
(define-public (finalize-proposal
  ...
  (let ((proposal (unwrap! (map-get? proposals proposal-id) ERR_BUDGET_NOT_FOUND)))
    (begin
      ...
      (map-set proposals proposal-id
        (merge proposal { status: (if approve
          "approved"
          "rejected"
        ) }
        ))
...
(define-public (record-expenditure
  ...
  (let (
      (budget (unwrap! (map-get? budgets budget-id) ERR_BUDGET_NOT_FOUND))
